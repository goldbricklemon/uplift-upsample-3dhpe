...
from common.dataset.camera import normalize_screen_coordinates

from common.dataset.keypoint_order import H36MOrderFull

_point_names = ['pelvis', 'hip_r', 'knee_r', 'heel_r', 'foot_r', 'toes_r', 'hip_l', 'knee_l', 'heel_l', 'foot_l',
                'toes_l', 'same_as_pelvis', 'torso', 'neck', 'head', 'brain', 'same_as_neck',
                'shoulder_l', 'elbow_l', 'wrist_l', 'same_as_wrist_l', 'thumb_l',
                'fingers_l', 'same_as_fingers_l', 'same_as_neck_2', 'shoulder_r', 'elbow_r',
                'wrist_r', 'same_as_wrist_r', 'thumb_r', 'fingers_r', 'same_as_fingers_r']
    ...
    def __init__(self, path):
        ...
        for cameras in self._cameras.values():
            for i, cam in enumerate(cameras):
                ...
                # Add combined intrinsic parameters vector
                # 2+2+2+3+2 = 11 values
                cam['intrinsic'] = np.concatenate(([cam['res_w'], cam['res_h']],
                                                   cam['focal_length'],
                                                   cam['center'],
        ...
        for subject, actions in data.items():
            ...
            for action_name, positions in actions.items():
                # Reduce to 17 point order
                positions = positions[:, H36MOrderFull.to_17p_order()].copy()

                ##############################################################
                # NOTE: Data is in xyz format, with x=right, y=forward, z=up
                ##############################################################
